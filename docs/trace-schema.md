...
- **校验框架**：Jarvis Studio 导入端（[traceImportService](file:///Users/jhx/Documents/learn/mark-ai/mark-series/mark-zero-jarvis/jarvis_studio/apps/server/src/services/traceImportService.ts)）使用 [Zod](https://zod.dev/) 对导入的每一行进行验证。不符合规范的行会导致导入失败并回滚。
