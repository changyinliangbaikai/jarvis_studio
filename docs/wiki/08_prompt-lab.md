...
2. **事件模拟注入**：后台（[promptService.ts](file:///Users/jhx/Documents/learn/mark-ai/mark-series/mark-zero-jarvis/jarvis_studio/apps/jarvis-studio-server/src/services/promptService.ts)）会自动发起一系列模拟事件（包含 `run.start`、`turn.start`、`context.build`、`llm.call`、`run.end`），将渲染好的 Full Prompt 作为上下文快照载入，并在 `context.build` 的 Payload 中真实注入。
...
---
[👈 上一页：07_测试任务对比与发布准入](07_task-compare-and-gate.md) | [首页](00_README.md) | [下一页：09_重放调试与实验矩阵 ➡️](09_replay-and-experiments.md)
