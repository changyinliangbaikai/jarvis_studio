...
export function PlaygroundPage() {
  ...
  const [inputMessage, setInputMessage] = useState(t('pages.playground.string_17'));
  ...
  const run = async () => {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : t('pages.playground.string_18'));
    } finally {
      setRunning(false);
    }
  };
  ...
  return <section>
    <PageHeader eyebrow="03 / Playground" title="Playground" description={t('pages.playground.string_1')} actions={<button className="primary" disabled={running || !promptId} onClick={() => void run()}><PlayCircle size={15} />{running ? t('common.running') : t('pages.playground.string_22')}</button>} />
    ...
    {agents.length === 0 || prompts.length === 0 ? <Empty>{t('pages.playground.string_2')}</Empty> : <div className="workbench-grid">
      <div className="panel editor-panel">
        <div className="panel-title">{t('pages.playground.string_3')}<span>Prompt-first</span></div>
        <div className="mini-form">
          ...
          <label>{t('pages.playground.string_4')}<textarea rows={7} value={inputMessage} onChange={(event) => setInputMessage(event.target.value)} /></label>
          <label>{t('pages.playground.string_5')}<textarea rows={6} value={variablesText} onChange={(event) => setVariablesText(event.target.value)} /></label>
        </div>
      </div>
      <div className="panel editor-panel">
        <div className="panel-title">{t('pages.playground.string_6')}<span>{result?.runId ?? t('pages.playground.string_23')}</span></div>
        {!result ? <Empty>{t('pages.playground.string_7')}</Empty> : <>
          <div className="editor-toolbar"><div><span className="eyebrow">Assistant Output</span><h2>{t('pages.playground.string_8')}</h2></div><Link className="primary-link" to={`/runs/${result.runId}`}><ExternalLink size={14} />{t('pages.playground.string_9')}</Link></div>
