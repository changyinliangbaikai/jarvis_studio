...
export function ComparePage() {
  ...
  return <section><PageHeader eyebrow={t('pages.compare.string_1')} title={t('pages.compare.string_2')} description={t('pages.compare.string_3')} />
    {runResource.error && <div className="notice warning">{runResource.error}</div>}
    <div className="compare-controls panel"><label>{t('pages.compare.string_10')}<ThemedSelect value={left} onChange={(event) => setLeft(event.target.value)}>{runs.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</ThemedSelect></label><ArrowRight size={22} /><label>{t('pages.compare.string_11')}<ThemedSelect value={right} onChange={(event) => setRight(event.target.value)}>{runs.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</ThemedSelect></label><button className="primary" disabled={!left || !right || left === right} onClick={() => void post<CompareResult>('/api/eval/compare', { baselineEvalRunId: left, candidateEvalRunId: right }).then(setResult)}><GitCompareArrows size={15} />{t('pages.compare.string_12')}</button></div>
    {!result ? <Empty>{t('pages.compare.string_13')}</Empty> : <><div className="comparison-head"><RunHead label={t('pages.compare.string_4')} item={result.baseline} /><div className="versus">VS</div><RunHead label={t('pages.compare.string_5')} item={result.candidate} /></div><div className="metric-grid"><Delta label={t('pages.compare.string_6')} value={result.delta.passRate} format={(value) => `${(value * 100).toFixed(1)}%`} positive /><Delta label={t('pages.compare.string_7')} value={result.delta.avgScore} format={(value) => value.toFixed(2)} positive /><Delta label={t('pages.compare.string_8')} value={result.delta.avgLatencyMs} format={formatDuration} /><Delta label={t('pages.compare.string_9')} value={result.delta.totalCost} format={(value) => value.toFixed(6)} /></div><div className="metric-grid compact"><Metric label="IMPROVED" value={result.summary.improved} tone="green" /><Metric label="REGRESSED" value={result.summary.regressed} tone="red" /><Metric label="UNCHANGED" value={result.summary.unchanged} /><Metric label="NEW FAILURES" value={result.summary.newFailures} tone="amber" /></div><div className="table-wrap"><table><thead><tr><th>{t('pages.compare.string_14')}</th><th>{t('pages.compare.string_15')}</th><th>{t('pages.compare.string_16')}</th><th>{t('pages.compare.string_17')}</th><th>Token Δ</th><th>{t('pages.compare.string_18')}</th><th>{t('pages.compare.string_19')}</th><th>{t('pages.compare.string_20')}</th></tr></thead><tbody>{result.cases.map((item) => <tr key={item.caseId}><td className="run-name"><strong>{item.name}</strong><span>{item.caseId}</span></td><td><StatusBadge status={item.classification} /></td><td className="score">{item.scoreDelta.toFixed(2)}</td><td>{formatDuration(item.latencyDelta)}</td><td>{formatNumber(item.tokensDelta)}</td><td>{item.costDelta.toFixed(6)}</td><td>{item.baseline && <Link to={`/evals/results/${item.baseline.id}`}>{item.baseline.totalScore.toFixed(2)} ↗</Link>}</td><td>{item.candidate && <Link to={`/evals/results/${item.candidate.id}`}>{item.candidate.totalScore.toFixed(2)} ↗</Link>}</td></tr>)}</tbody></table></div></>}
  </section>;
}
