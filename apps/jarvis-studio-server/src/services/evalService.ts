...
import { callModel, type ModelProfile, type RuntimeResult, type SkillDefinition } from '@jarvis/agent-runtime';
import {
  evalCaseSchema,
  evalDatasetSchema,
  type EvalCase,
  type EvalDataset
} from '@jarvis/eval-runner';
import { evaluateReleaseGate, issueTags, scoreRules, type RuleScore } from '@jarvis/eval-runner';
...
import { recordFailure } from './failureService.ts';
...
function normalizeCaseResult(row: Record<string, unknown>) {
  return {
    ...
    judgeResult: parseJson<Record<string, unknown> | null>(row.judge_result_json, null),
    humanReview: parseJson<Record<string, unknown> | null>(row.human_review_json, null), finalOutput: row.final_output,
...
function createEvalRunRecord(input: CreateEvalRunInput) {
  ...
  const config = normalizedRunConfig({ ...input, modelProviderId: provider.id, modelName }, provider.profile.provider);
  ...
  promptVersion, input.skillVersion ?? null, 'agent-runtime@0.5.0', 'jarvis-runtime-tools@v0.5',
  input.contextStrategy ?? 'balanced-v1', process.env.CODE_COMMIT_HASH ?? null, selectedCases, provider.pricing.currency,
  ...
  json(config), json({
    ...
    toolSchemaHash: versionHash('jarvis-runtime-tools@v0.4'),
    contextStrategyHash: versionHash(input.contextStrategy ?? 'balanced-v1'),
    toolPolicyHash: versionHash(input.toolPolicy ?? 'default-local-policy@0.4.0'),
  ...
  try {
    const runtime = await executeRuntimeWithRetry(
      () => withTimeout(executeRuntimeRun({
        ...
        contextStrategy: typeof runtimeOverrides.contextStrategy === 'string' ? runtimeOverrides.contextStrategy : config.contextStrategy ?? 'balanced-v1',
        policyVersion: typeof runtimeOverrides.toolPolicy === 'string' ? runtimeOverrides.toolPolicy : config.toolPolicy ?? 'default-local-policy@0.4.0',
    ...
    const runtimeIssueTags = runtime.status === 'success' ? [] : runtimeFailureTags(runtime.error);
    if (runtime.status !== 'success') {
      logEvalModelFailure(new Error(runtime.error ?? 'Runtime 执行失败'), evalRunId, resultId, evalCase, config, effectiveProvider.id, profile.model);
    }
    ...
    const permissionDeniedCount = runtime.events.filter((event) =>
      (event.eventType === 'permission.check' || event.eventType === 'tool.policy.check') && (event.payload.status === 'denied' || event.payload.decision === 'deny')).length;
    ...
    const issueTagsForResult = [...new Set([...runtimeIssueTags, ...rule.issueTags])];
    ...
    cost, runtime.toolCalls.length, runtime.artifacts.length, json(issueTagsForResult), json(rule), json(judge),
    ...
    if (!passed) {
      recordEvalFailure(evalRunId, resultId, evalCase, runtime.ids.runId, issueTagsForResult, runtime.error);
    }
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    const tag = /timeout/i.test(message) ? 'model_timeout' : 'model_error';
    logEvalModelFailure(error, evalRunId, resultId, evalCase, config, effectiveProvider.id, profile.model);
    ...
    recordEvalFailure(evalRunId, resultId, evalCase, undefined, [tag], message);
  }
}
...
function parseJudgeJson(content: string): Record<string, unknown> {
...
${gate ? (gate.failedCriteria.length ? gate.failedCriteria.map((item: { label: string; actual: unknown; expected: unknown }) => `- ${item.label}: ${item.actual}，要求 ${item.expected}`).join('\n') : '- 所有门禁项通过') : '- 未执行 Release Gate'}
...
function normalizedRunConfig(input: CreateEvalRunInput, providerType?: ModelProfile['provider']): CreateEvalRunInput {
  const isRemoteProvider = providerType === 'openai-compatible';
  const requestedMaxParallel = Number(input.maxParallel ?? (isRemoteProvider ? 1 : 2));
  const requestedRetryCount = Number(input.retryCount ?? (isRemoteProvider ? 2 : 0));
  return {
    ...input,
    maxParallel: isRemoteProvider ? 1 : Math.min(Math.max(requestedMaxParallel, 1), 5),
    retryCount: isRemoteProvider ? Math.min(Math.max(requestedRetryCount, 2), 3) : Math.min(Math.max(requestedRetryCount, 0), 3),
    timeoutSeconds: Math.min(Math.max(Number(input.timeoutSeconds ?? 120), 1), 900),
    promptVersion: input.promptVersion ?? 'base-agent@v0.3',
    contextStrategy: input.contextStrategy ?? 'balanced-v1',
    toolPolicy: input.toolPolicy ?? 'default-local-policy@0.4.0'
  };
}

function logEvalModelFailure(error: unknown, evalRunId: string, resultId: string, evalCase: EvalCase, config: CreateEvalRunInput, providerId: string, model: string) {
  logModelProviderError('eval_case.runtime_failed', {
    evalRunId,
    resultId,
...
function recordEvalFailure(evalRunId: string, resultId: string, evalCase: EvalCase, runId: string | undefined, issueTags: string[], error?: string) {
  const primaryTag = issueTags[0] ?? (error ? 'model_error' : 'eval_failed');
  const type = failureTypeForTag(primaryTag);
  recordFailure({
    runId: runId ?? evalRunId,
    evalCaseId: evalCase.id,
    type,
    severity: evalCase.priority === 'p0' ? 'high' : 'medium',
    summary: error ?? `Eval Case ${evalCase.id} 未通过：${issueTags.join(', ') || 'score below threshold'}`,
    evidence: [{ evalRunId, evalCaseId: evalCase.id, resultId, issueTags }],
    suggestedFix: suggestedFixForTag(type),
    skillId: typeof evalCase.runtimeOverrides.skill === 'string' ? evalCase.runtimeOverrides.skill : undefined
  });
}

function failureTypeForTag(tag: string) {
  if (/skill/i.test(tag)) return 'skill_misroute';
  if (/tool/i.test(tag)) return 'tool_execution_error';
  if (/artifact/i.test(tag)) return 'artifact_missing';
  if (/timeout/i.test(tag)) return 'timeout';
  if (/permission/i.test(tag)) return 'permission_denied';
  if (/model/i.test(tag)) return 'model_reasoning_error';
  return 'eval_judge_error';
}

function suggestedFixForTag(type: string) {
  if (type === 'skill_misroute') return '检查 Skill 触发词、文件类型和用例 runtime_overrides.skill。';
  if (type === 'tool_execution_error') return '查看 Tool Registry、权限决策和工具参数证据。';
  if (type === 'artifact_missing') return '检查 Skill 输出文件声明、工具链和产物路径。';
  if (type === 'permission_denied') return '在 Approvals 或 Tool Policy 中处理被拒绝的工具调用。';
  if (type === 'timeout') return '降低并发、增加 timeoutSeconds 或压缩上下文。';
  return '复查 Eval 期望、评分规则、Prompt 和最终输出。';
}

function runtimeFailureTags(error?: string) {
  if (/timeout/i.test(error ?? '')) return ['model_timeout'];
  if (/permission|拒绝|审批/i.test(error ?? '')) return ['permission_denied'];
  return ['model_error'];
}

function matrixCombinations(input: CreateEvalRunInput) {
  if (!input.matrix) return [input];
  ...
  const skills = input.matrix.skillVersions?.length ? input.matrix.skillVersions : [input.skillVersion];
  const contexts = input.matrix.contextStrategies?.length ? input.matrix.contextStrategies : [input.contextStrategy];
  const toolPolicies = input.matrix.toolPolicies?.length ? input.matrix.toolPolicies : [input.toolPolicy];
  ...
  return providers.flatMap((modelProviderId) => models.flatMap((modelName) => prompts.flatMap((promptVersion) =>
    skills.flatMap((skillVersion) => contexts.flatMap((contextStrategy) => toolPolicies.flatMap((toolPolicy) =>
      temperatures.map((temperature) => ({ ...input, matrix: undefined, modelProviderId, modelName, promptVersion, skillVersion, contextStrategy, toolPolicy, temperature }))))))));
}
...
async function executeRuntimeWithRetry(action: () => Promise<RuntimeResult>, retries: number) {
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    ...
    if (attempt < retries) await delay(Math.min(500 * 2 ** attempt, 2_000));
  }
