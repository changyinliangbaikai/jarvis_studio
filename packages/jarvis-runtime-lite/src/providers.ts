...
import type { ModelMessage, ModelProfile, ModelResponse, ModelToolCall, SkillDefinition, ToolDefinition } from './types.ts';
...
async function callOpenAICompatible(input: Parameters<typeof callModel>[0]): Promise<ModelResponse> {
  ...
  const toolCalls: ModelToolCall[] = (message?.tool_calls ?? []).flatMap((call) => {
    ...
    try {
      return [{ id: call.id ?? `call_${randomUUID()}`, name: externalToolNames.get(call.function.name) ?? call.function.name, arguments: JSON.parse(call.function.arguments ?? '{}') as Record<string, unknown> }];
    } catch {
      return [{ id: call.id ?? `call_${randomUUID()}`, name: externalToolNames.get(call.function.name) ?? call.function.name, arguments: {} }];
    }
  });
